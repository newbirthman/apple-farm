...
function mapIncomingRow(row: any): IncomingRecord {
    return {
        ...
        packagingStatus: row.packaging_status || undefined,
...
export function useInventory() {
    ...
    const updatePrice = useCallback(async (id: string, newPrice: number) => {
        ...
        const { error } = await supabase
            ...
            .eq('id', id);
        if (error) console.error('단가 수정 오류:', error);
    }, []);
    ...
    const addIncoming = useCallback(async (record: Omit<IncomingRecord, 'id'>) => {
        const row = {
            ...
            packaging_status: record.packagingStatus || null,
            quantity: record.quantity,
            unit_price: record.unitPrice || null,
            total_price: record.totalPrice || null,
        };
        const { data, error } = await supabase.from('incoming_records').insert(row).select().single();
        if (error) throw new Error(error.message);
        if (data) setIncoming(prev => [mapIncomingRow(data), ...prev]);
    }, []);
    ...
    const addSales = useCallback(async (record: Omit<SalesRecord, 'id'>) => {
        ...
        if (error) throw new Error(error.message);
        if (data) setSales(prev => [mapSalesRow(data), ...prev]);
    }, []);

    // ─── 새 상품(단가) 등록 ───
    const addPriceItem = useCallback(async (record: { cropType: string, category: string, itemName: string, price: number }) => {
        const row = { crop_type: record.cropType, category: record.category, item_name: record.itemName, price: record.price };
        const { data, error } = await supabase.from('prices').insert(row).select().single();
        if (error) throw new Error(error.message);
        if (data) setPrices(prev => [...prev, mapPriceRow(data)]);
    }, []);
    ...
    const updateDeliveryFee = useCallback(async (fee: number) => {
        setDeliveryFee(fee);
        const { error } = await supabase.from('app_settings').upsert({ key: 'delivery_fee', value: fee.toString() });
    ...
    const updateDeliveryFeeIsland = useCallback(async (fee: number) => {
        ...
        const { error } = await supabase.from('app_settings').upsert({ key: 'delivery_fee_island', value: fee.toString() });
        if (error) console.error('도서산간 택배비 수정 오류:', error);
    }, []);

    return {
        prices, incoming, sales, deliveryFee, deliveryFeeIsland, customers, isLoading,
        updatePrice, getPrice, addIncoming, addSales, addPriceItem, deletePriceItem,
        upsertCustomer, deleteCustomer, updateDeliveryFee, updateDeliveryFeeIsland,
    };
}
